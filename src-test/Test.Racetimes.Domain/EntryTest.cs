...
namespace Test.Racetimes.Domain
{
    public class EntryTest
    {
        ...
        private static async Task<CompetitionId> PrepareCompetition(EventFlow.Configuration.IRootResolver resolver, string name, string user, CompetitionId competitionId = null)
        {
            ...
            // Create
            var executionResult = await commandBus.PublishAsync(new RegisterCompetitionCommand(domainId, user, name), CancellationToken.None);
            ...
            var readModel1 = await queryProcessor.ProcessAsync(new ReadModelByIdQuery<CompetitionReadModel>(domainId), CancellationToken.None);
        ...
        public async Task RecordEntryTest(string discipline, string competitor, int time, bool expectedResult)
        {
            using var resolver = New
                ...
                .CreateResolver();
            var domainId = await PrepareCompetition(resolver, "test-competition", "test-user");
            ...
            // Rename
            var executionResult = await commandBus.PublishAsync(new RecordEntryCommand(domainId, entryId, discipline, competitor, time), CancellationToken.None);
            executionResult.IsSuccess.Should().Be(expectedResult);

            // Verify that the read model has the expected value
            var readModel2 = await queryProcessor.ProcessAsync(new ReadModelByIdQuery<EntryReadModel>(entryId), CancellationToken.None);
        ...
        public async Task CorrectEntryTimeTest(int time1, int time2, bool expectedResult)
        {
            using var resolver = New
            ...
            var domainId = await PrepareCompetition(resolver, name, user);
            var entryId = EntryId.New;
            ...
            // Rename
            var executionResult = await commandBus.PublishAsync(new RecordEntryCommand(domainId, entryId, discipline, competitor, time1), CancellationToken.None);
            executionResult.IsSuccess.Should().BeTrue();
            ...
            executionResult = await commandBus.PublishAsync(new CorrectEntryTimeCommand(domainId, entryId, time2), CancellationToken.None);
            executionResult.IsSuccess.Should().Be(expectedResult);

            var readModel3 = await queryProcessor.ProcessAsync(new ReadModelByIdQuery<EntryReadModel>(entryId), CancellationToken.None);
        ...
        [Fact]
        public async Task CorrectEntryTimeOnMissingEntry()
        {
            using var resolver = New
            ...
            // Preparations finished: Start test
            var executionResult = await commandBus.PublishAsync(new CorrectEntryTimeCommand(domainId, entryId, time), CancellationToken.None);
        ...
        [InlineData(2)]
        [InlineData(10)]
        public async Task DeleteCompetitionTest(int amount)
        {
            using var resolver = New
            ...
            EntryId[] ids = await RegisterCompetitionWithEntries(resolver, amount, domainId);

            var commandBus = resolver.Resolve<ICommandBus>();
            var queryProcessor = resolver.Resolve<IQueryProcessor>();

            // Delete
            var executionResult = await commandBus.PublishAsync(new DeleteCompetitionCommand(domainId), CancellationToken.None);
            executionResult.IsSuccess.Should().BeTrue();
        ...
        [InlineData(3)]
        [InlineData(10)]
        public async Task CreateSnapshotTest(int amount)
        {
            using var resolver = New
                .AddEvents(typeof(CompetitionRegisteredEvent), typeof(CompetitionDeletedEvent), typeof(EntryRecordedEvent), typeof(EntryTimeCorrectedEvent))
                .AddCommands(typeof(RegisterCompetitionCommand), typeof(DeleteCompetitionCommand), typeof(RecordEntryCommand), typeof(CorrectEntryTimeCommand))
                ...
                .CreateResolver();
            await RegisterCompetitionWithEntries(resolver, amount);
        }

        private static async Task<EntryId[]> RegisterCompetitionWithEntries(IRootResolver resolver, int amount, CompetitionId competitionId = null)
        {
             var domainId = await PrepareCompetition(resolver, "test-competition", "test-user", competitionId);
            ...
            for (int x = 0; x < amount; x++)
            {
                ...
                // Add
                var executionResult = await commandBus.PublishAsync(new RecordEntryCommand(domainId, entryId, discipline, competitor, time + 1), CancellationToken.None);
                executionResult.IsSuccess.Should().BeTrue();

                // Change time
                executionResult = await commandBus.PublishAsync(new CorrectEntryTimeCommand(domainId, entryId, time), CancellationToken.None);
                executionResult.IsSuccess.Should().BeTrue();
