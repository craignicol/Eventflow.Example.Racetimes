...
namespace Test.Racetimes.Domain
{
    public class CompetitionTest
    {
        ...
        public async Task RegisterTest(string name, string user, bool expectedResult)
        {
            ...
            var readModel = await queryProcessor.ProcessAsync(new ReadModelByIdQuery<CompetitionReadModel>(domainId), CancellationToken.None);
        ...
        [InlineData("", false)]
        [InlineData(null, false)]
        public async Task CorrectTest(string name2, bool expectedResult)
        {
            using var resolver = New
            ...
            // Create
            var executionResult = await commandBus.PublishAsync(new RegisterCompetitionCommand(domainId, user, name1), CancellationToken.None);
            executionResult.IsSuccess.Should().BeTrue();

            // Rename
            executionResult = await commandBus.PublishAsync(new CorrectCompetitionCommand(domainId, name2), CancellationToken.None);
            executionResult.IsSuccess.Should().Be(expectedResult);
        ...
        public async Task DeleteTest()
        {
            ...
            // Delete
            executionResult = await commandBus.PublishAsync(new DeleteCompetitionCommand(domainId), CancellationToken.None);
            executionResult.IsSuccess.Should().BeTrue();
        ...
        public async Task CreateSnapshotTest(params string[] names)
        {
            ...
            foreach (string name2 in names)
            {
                ...
                readModel = await queryProcessor.ProcessAsync(new ReadModelByIdQuery<CompetitionReadModel>(domainId), CancellationToken.None);
